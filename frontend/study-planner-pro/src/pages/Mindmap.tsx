  ...
  const loadAllPlans = async () => {
    ...
    try {
      ...
      const { data: plans, error: fetchError } = await supabase
        .from('study_plans')
        .select('id, subject, syllabus_content')
        .eq('completed', false)
        .gte('test_date', today)
        ...
        {loading ? (
          ...
          <Card className="bg-card">
            ...
            <CardContent>
              {/* Progress Section */}
              {(() => {
                // Count all topics (main topics + subtopics)
                const allTopics = flowchartData.mainTopics.reduce((acc, topic) => {
                  return acc + 1 + topic.subtopics.length; // 1 for main topic + all subtopics
                }, 0);
                
                // Count achieved topics (main + subtopics)
                const achievedCount = Object.values(achievements).filter(Boolean).length;
                
                const totalTopics = allTopics;
                const achievedTopics = achievedCount;
                  ...
                  <div className="mb-6 p-5 bg-gradient-to-r from-primary/10 to-accent/10 rounded-lg border-2 border-primary/20">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm text-muted-foreground">
                        <span>{achievedTopics} of {totalTopics} topics achieved</span>
                      ...
                      {percentage === 100 && (
                        <div className="mt-3 p-2 bg-green-500/10 border border-green-500/20 rounded text-center">
                          <span className="text-green-700 font-semibold">🎉 All topics mastered!</span>
                        </div>
                      )}
                    </div>
                  </div>
